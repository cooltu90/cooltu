...
import cooltu.processor.modelinterface.StartMethodModelInterface;
        ...
        Ts.ls(kvs, new Each<KV<String, String>>() {
            ...
            public boolean each(int position, KV<String, String> kv) {
                if (ClassTool.isBaseClass(kv.k)) {
                    addLnTag(sb, "        intent.putExtra(Pass.[NAME], [name]);", NameTools.toStaticType(kv.v), kv.v);
                } else {
                    addLnTag(sb, "        intent.putExtra(Pass.[NAME], [JsonTools].toJson([name]));",
                            NameTools.toStaticType(kv.v), FullName.JSON_TOOL, kv.v);
                }
                return false;
            }
        });
