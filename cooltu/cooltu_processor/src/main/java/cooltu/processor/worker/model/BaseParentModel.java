...
import cooltu.processor.lib.bean.FromItemInfoForRg;
import cooltu.processor.lib.log.Logs;
...
import cooltu.processor.lib.tools.LayoutTools;
import cooltu.processor.lib.tools.NameTools;
import cooltu.processor.lib.tools.ParamTools;
import cooltu.processor.lib.tools.RenameTools;
import cooltu.processor.lib.tools.TagTools;
...
public abstract class BaseParentModel extends BaseModel {
        ...
        Ts.ls(vis, new Each<LayoutTools.ViewInfo>() {
            ...
            public boolean each(int position, LayoutTools.ViewInfo vi) {
                ...
                addLnTag(findViewSb, "        [n1] = [n2]findViewById([n3].R.id.[n4]);",
                        ...
                        isCoreR ? Constant.PKG_CORE_LIB4A : layoutId.rPackage,
                        vi.id);
                return false;
            }
        });
    ...
    public void setTagFor_bindCheck(StringBuilder bindCheckSb) {
        if (formBeanKv != null) {
            addLnTag(bindCheckSb, "    protected boolean check[User]() {", NameTools.toClassType(formBeanKv.v));
            ...
            public boolean each(int position, ExecutableElement element) {
                ...
                for (int i = 0; i < CountTool.count(ids); i++) {
                    ...
                    if (clickView.check() && formBeanKv != null) {
                        addLnTag(sb, "                if (!check[User]()) {", NameTools.toClassType(formBeanKv.v));
                        addLnTag(sb, "                    return;");
                        addLnTag(sb, "                }");
                    }
                }
                ...
                addLnTag(sb, "                [methodName](", NameTools.simpleName(element));
            ...
            public boolean each(int position, ExecutableElement element) {
                ...
                for (int i = 0; i < CountTool.count(ids); i++) {
                    ...
                    if (clickView.check() && StringTool.isNotBlank(beanName)) {
                        addLnTag(sb, "                if (!check[User]()) {", NameTools.toClassType(beanName));
                        addLnTag(sb, "                    return false;");
                        addLnTag(sb, "                }");
                    }
                }

                addLnTag(sb, "                return [methodName](", NameTools.simpleName(element));
